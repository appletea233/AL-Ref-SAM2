...
def sub_processor(lock, pid, cfg, sub_df_v2_test):
    text = 'processor %d' % pid
    with lock:
        progress = tqdm(
            total=len(sub_df_v2_test),
            position=pid,
            desc=text,
            ncols=0
        )
    ...
    for index in range(len(sub_df_v2_test)):
        df_one_video = sub_df_v2_test.iloc[index]
        ...
        for label, keyframe_bboxes in label_keyframe_bboxes.items():
            ...
            img_path = os.path.join(img_dir, video_name, label, "box.jpg")
...
if __name__ == "__main__":
    ...
    for i in range(thread_num):
        if i == thread_num - 1:
            sub_df_v2_test = df_v2_test.iloc[i * per_thread_video_num:]
        else:
            sub_df_v2_test = df_v2_test.iloc[i * per_thread_video_num: (i + 1) * per_thread_video_num]
        p = mp.Process(target=sub_processor, args=(lock, i, cfg, sub_df_v2_test))
        p.start()
        processes.append(p)
