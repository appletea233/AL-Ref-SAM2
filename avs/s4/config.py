...
cfg.CATEGORIES = ['baby_laughter', 'female_singing', 'male_speech', 'dog_barking',
                  'cat_meowing', 'lions_roaring', 'horse_clip-clop', 'coyote_howling',
                  'mynah_bird_singing', 'playing_acoustic_guitar', 'playing_tabla',
                  'playing_violin', 'playing_piano', 'playing_ukulele', 'playing_glockenspiel',
                  'helicopter', 'race_car', 'driving_buses', 'ambulance_siren', 'typing_on_computer_keyboard',
                  'chainsawing_trees', 'cap_gun_shooting', 'lawn_mowing'] # 23 categories
...
# data
cfg.DATA = edict()
cfg.DATA.BASE_DIR = "YOUR AVSBench Single-source DIR" # TODO
cfg.DATA.ANNO_CSV = os.path.join(cfg.DATA.BASE_DIR, "s4_meta_data.csv")
cfg.DATA.PNG_IMG_DIR = os.path.join(cfg.DATA.BASE_DIR, "s4_data", "visual_frames", cfg.SPLIT)
cfg.DATA.AUDIO_WAV_DIR = os.path.join(cfg.DATA.BASE_DIR, "s4_data", "audio_wav", cfg.SPLIT)
cfg.DATA.GT_MASK_DIR = os.path.join(cfg.DATA.BASE_DIR, "s4_data", "gt_masks", cfg.SPLIT)
