...
if __name__ == '__main__':
    ...
    for index in range(len(df_test)):
        ...
        frame_dir = os.path.join(cfg.DATA.PNG_IMG_DIR, category, video_name)
        for img_id in range(1, T + 1):
            png_path = os.path.join(frame_dir, f"{video_name}_{img_id}.png")
            rgb_image = Image.open(png_path).convert('RGB')
            jpg_dir = os.path.join(jpg_base_dir, category, video_name)
