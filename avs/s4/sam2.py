...
if __name__ == '__main__':
    ...
    for index in range(len(df_test)):
        ...
        origin_w, origin_h = Image.open(os.path.join(cfg.DATA.JPG_IMG_DIR, category, video_name, "1.jpg")).convert('RGB').size
        save_dir = os.path.join(cfg.SAM2.VIS_OUTPUT_DIR, category, video_name)
        os.makedirs(save_dir, exist_ok=True)

        label_keyframe_bboxes = name_label_keyframe_bboxes[video_name]
        if len(label_keyframe_bboxes) == 0:
            all_pred_masks = torch.zeros(T, origin_h, origin_w, dtype=torch.bool)
        else:
            ...
            if len(bboxes) == 0:
                ...
                box_id = 0 if len(bboxes) == 1 else (int(name_keybox[video_name]) - 1)
                ...
                frame_dir = os.path.join(cfg.DATA.JPG_IMG_DIR, category, video_name)
                frame_path = os.path.join(frame_dir, f"{keyframe}.jpg")
