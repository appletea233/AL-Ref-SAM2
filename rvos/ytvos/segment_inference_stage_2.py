...
def main(args):
    ...
    save_path_prefix = os.path.join(output_dir, 'output_stage_2', split)
...
def sub_processor(lock, pid, args, data, save_path_prefix, img_folder, video_list):
    ...
    for video in video_list:
        ...
        for i in range(num_expressions):
            ...
            cot_json_dir = os.path.join(concat_video_dir, 'cot_two_stage.json')

            save_path = os.path.join(save_path_prefix, video_name, exp_id)
            if os.path.exists(save_path):
                continue

            with open(cot_json_dir, 'r') as f:
                results = json.load(f)
            key_frames = []
            ...
            for t, segment in enumerate(results['res_list']):
                ...
                if real_freame_id >= len(frames):
                    continue
                target_object_id = segment.get('target_object_id', -1)

                boxes = torch.tensor(segment['boxes_info']['boxes'])
                if target_object_id <= 0 or target_object_id > boxes.shape[0]:
                    continue
                ...
                input_box = input_boxes[target_object_id - 1]
